Shader "Full Sail/Sails Mobile Unlit"
{
	Properties
	{
	...
	SubShader
	{
		...
		Pass
		{
			...
			v2f vert(appdata v)
			{
				...
				o.texcoord1 = v.texcoord1;	// TRANSFORM_TEX(v.uv, _MainTex);
				o.normal = v.normal;
			...
			{
				float dam = _Damage;	// UNITY_ACCESS_INSTANCED_PROP(Props, _Damage);
				fixed4 c;
	
				if ( dam > 0 )
				{
					if ( dam < 0.333 )
						c = lerp(tex2D(_MainTex, IN.texcoord1), tex2D(_DamageTex, IN.texcoord1), dam / 0.333);
					else
					{
						if ( dam < 0.666 )
							c = lerp(tex2D(_DamageTex, IN.texcoord1), tex2D(_DamageTex1, IN.texcoord1), (dam - 0.333) / 0.333);
						else
						{
							float la = (dam - 0.666) / 0.333;

							la = clamp(la, 0.0, 1.25);
							c = lerp(tex2D(_DamageTex1, IN.texcoord1), tex2D(_DamageTex2, IN.texcoord1), la);
						}
					}
				}
				else
					c = tex2D(_MainTex, IN.texcoord1);

				clip(c.a - _Cutoff);

				float dot1 = dot(IN.normal.xyz, _SailForward.xyz);

				float2 euv = IN.texcoord1;
				//euv.x = 1.0 - euv.x;
				...
				{
					float2 delta = IN.texcoord1.xy - _ImpactPoints[i].xy;
